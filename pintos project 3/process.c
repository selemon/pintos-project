...
process_exit (void)
{
  ...
  //my code
  for(element = list_begin(&cur->children); element!=list_end(&cur->children); element = next){
    struct wait_status *cs = list_entry(element, struct wait_status, elem);
    next = list_remove(element);
    release_child(cs);
  }
